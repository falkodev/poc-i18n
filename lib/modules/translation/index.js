...
module.exports = {
  ...
  async construct(self, options) {
    // languages.forEach(lang => self.apos.i18n.addLocale(lang.value))
    // self.apos.i18n.setLocale(config.get('languages.default'))

    // self.addHelpers({
    //   t: (key, number) => {
    //     const translation = number ? self.apos.i18n.__n(key, number) : self.apos.i18n.__(key)
    //     const compare = translation.replace(/(\d+ | \d+)/, ' %s ').trim() // replace numbers with "%s" to compare with i18n key
    //     if (compare === key && !missingKeys.find(piece => piece.key === key)) {
    //       missingKeys.push({
    //         type: self.name,
    //         title: key,
    //         key: key,
    ...
    self.beforeInsert = async (req, piece, options, callback) => {
      try {
        const keyAlreadyExists = await self.find(req, { key: piece.key, lang: piece.lang }, { permissions: false }).toObject()
    ...
    self.afterSave = async (req, piece, options, callback) => {
      try {
        ...
        const pieces = await self
          .find(
            req,
            { published: true, lang: piece.lang },
            { key: 1, valueSingular: 1, valuePlural: 1 },
          )
          .toArray()

        const translations = translatePieces(pieces)
        console.log('translations', require('util').inspect(translations, { colors: true, depth: 1 }))
